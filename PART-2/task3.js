function seleksiNilai(nilaiAwal, nilaiAkhir, dataArray) {
  ...
  if (!Array.isArray(dataArray)) {
    //jika dataArray tidak bertipe Array
    return "Nilai harus Array";
  }
  if (nilaiAwal > nilaiAkhir) {
    //jika nilaiAwal > nilai akhir
    return "Nilai awal harus lebih kecil dari nilai akhir";
  }
  if (dataArray.length < 5) {
    //jika nilai data array berjumlah < 5
    return "Jumlah angka dalam dataArray harus lebih dari 5";
  }
  const find = [];
  dataArray.forEach(function (element) {
    if (element > nilaiAwal && element < nilaiAkhir) {
      // untuk mencari scr looping bila nilaiAkhir < element > nilaiAwal
  ...
  if (urutFind.length === 0) {
    //kalau tidak ada nilai didalam urutFind, maka tampilkan stetment
